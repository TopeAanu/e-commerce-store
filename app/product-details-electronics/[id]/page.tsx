// app/product-details-electronics/[id]/page.tsx
"use client";
...
import { AddToCartButton } from "../../components/add-to-cart-icon"; // Fixed import path
import type { Product } from "../../lib/types"; // Fixed import path
import { useRouter } from "next/navigation";
import { ArrowLeft, Star, StarHalf, Heart } from "lucide-react";
...
interface ElectronicsDetailsPageProps {
  params: Promise<{ id: string }>;
}

const ElectronicsDetailsPage = ({ params }: ElectronicsDetailsPageProps) => {
  ...
  useEffect(() => {
    const resolveParams = async () => {
      try {
        ...
        console.log("Product ID from params:", resolvedParams.id); // Debug log
      } catch (err) {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        ...
        if (docSnap.exists()) {
          console.log("Found product by document ID"); // Debug log
          const data = docSnap.data() as FirestoreProduct;
          const productData = convertToProduct(data, docSnap.id);
          setProduct(productData);
          setError(null);
        } else {
          console.log("Product not found by document ID, trying by field ID"); // Debug log
          ...
          const queries = [
            ...
            query(productsRef, where("id", "==", parseInt(productId))),
          ];
          ...
          for (const q of queries) {
            try {
              ...
              if (!querySnapshot.empty) {
                console.log("Found product by field ID"); // Debug log
          ...
          if (!foundProduct) {
            console.log("Product not found anywhere"); // Debug log
          ...
          <div className="space-y-6">
            ...
            <div>
              <div className="text-sm text-gray-500 dark:text-gray-400 mb-2 capitalize">
                {product.category} •{" "}
                {product.description?.split(" - ")[1] || "Electronics"}
              </div>
